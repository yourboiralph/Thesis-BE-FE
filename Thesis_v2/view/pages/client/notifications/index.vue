...
async function fetchDetection(){
    ...
    try {
        ...
        if (Array.isArray(response.orders)) {
            // If the response has the orders array directly
            data.value = response.orders.map((item, index) => ({
                id: item.id,
                date: item.date,
            ...
            // If the response has the orders array nested under a 'data' field
        ...
        state.value.errors = error.response;
